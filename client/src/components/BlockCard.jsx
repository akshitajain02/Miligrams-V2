  ...
  return (
    <div className="block-card">
      <div className="block-card-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', color: 'var(--text-muted)', fontSize: '0.76rem', fontFamily: 'JetBrains Mono, monospace' }}>
          <Clock size={13} />
      ...
      <div className="block-field">
        ...
        <div className="hash-pill" style={{ flex: 1, background: 'var(--surface-alt)', color: 'var(--text-muted)', borderColor: 'var(--border)', justifyContent: 'space-between' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
            <Link2 size={13} color="var(--text-light)" />
      ...
      {block.data && (
        <div style={{ marginTop: '0.85rem', paddingTop: '0.75rem', borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.5rem', background: 'var(--surface-alt)', border: '1px solid var(--border)', padding: '0.85rem', borderRadius: '8px', fontSize: '0.82rem' }}>
            {Object.entries(block.data).map(([k, v]) => {
              ...
              return (
                <div key={k} style={{ background: isHighlighted ? 'rgba(16, 185, 129, 0.15)' : 'transparent', padding: '0.2rem', borderRadius: '4px' }}>
                  <span style={{ color: 'var(--text-light)', display: 'block', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase' }}>{k}</span>
                  <span style={{ color: isHighlighted ? '#34d399' : 'var(--text-main)', fontFamily: typeof v === 'number' ? 'JetBrains Mono' : 'inherit', wordBreak: 'break-all', fontWeight: isHighlighted ? 700 : 500 }}>
